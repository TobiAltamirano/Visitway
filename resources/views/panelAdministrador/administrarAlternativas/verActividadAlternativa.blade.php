...
<section>
    ...
    <section class="w-full py-12 md:py-20 lg:py-12">
            ...
            <div class="grid grid-cols-1 gap-8 sm:grid-cols-1 lg:grid-cols-1">
                <div class="grid gap-4 rounded-lg border bg-white p-6 shadow-sm transition-all hover:shadow-lg dark:bg-gray-950">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        ...
                        <img src="<?= url('storage/' . $actividadAlternativa->imagen2); ?>" alt="Actividad Imagen 2" class="rounded-lg object-cover w-full h-full modal-image" style="aspect-ratio: 300 / 200; object-fit: cover; cursor: pointer;" data-twe-toggle="tooltip" title="Ver imagen" />
                        @else
                        <!-- No se carga ninguna imagen -->
                        @endif

                        @if($actividadAlternativa->imagen3)
                        <img src="<?= url('storage/' . $actividadAlternativa->imagen3); ?>" alt="Actividad Imagen 3" class="rounded-lg object-cover w-full h-full modal-image" style="aspect-ratio: 300 / 200; object-fit: cover; cursor: pointer;" data-twe-toggle="tooltip" title="Ver imagen" />
                        @else
                        <!-- No se carga ninguna imagen -->
                        @endif
                    </div>
        ...
        <div class="flex justify-center mt-4 gap-4">
            <div>
                <form action="{{ route('administrador.actividades-alternativas.eliminar', ['id' => $actividadAlternativa->id]) }}" method="POST">
                    @csrf
                    @method('POST')
