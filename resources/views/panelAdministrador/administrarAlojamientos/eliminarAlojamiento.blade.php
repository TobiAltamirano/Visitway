...
@section('title', 'Panel Administrador - Eliminar Alojamiento')

@section('content')

<section>

    <h1 class="h1-provincias text-4xl text-2xl title-font mb-4 text-gray-900 rojo-secundario poppins-semibold">Eliminar alojamiento:<span class="span-h1-provincias"><br>{{ $alojamiento->nombre_alojamiento }}</span></h1>
    ...
    <p class="lg:w-2/3 mx-auto text-center font-bold leading-relaxed text-base rojo-principal roboto-flex mt-12">Estas a punto de eliminar este alojamiento. Esta acción no tiene vuelta atrás. <br> Por favor, asegúrate de que deseas continuar antes de proceder. </p>
    ...
    <div class="flex justify-center mt-8 gap-4">
        <div>
            <form action="{{ route('administrador.alojamientos.eliminar.procesar', ['id' => $alojamiento->id_alojamiento]) }}" method="POST">
                @csrf
                @method('POST')
                <button class="inline-flex h-10 sm:w-auto items-center justify-center rounded-md bg-gray-900 px-8 text-sm font-medium text-gray-50 shadow transition-colors hover:bg-gray-900/90 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-gray-950 disabled:pointer-events-none disabled:opacity-50 dark:bg-gray-50 dark:text-gray-900 dark:hover:bg-gray-50/90 dark:focus-visible:ring-gray-300 boton-eliminar roboto-flex" type="submit" data-id="27">Eliminar alojamiento</button>
            </form>
        </div>
        <a href="{{ route('administrador.alojamientos') }}" class="inline-flex h-10 sm:w-auto items-center justify-center rounded-md border border-gray-200 bg-white px-8 text-sm font-medium shadow-sm transition-colors hover:bg-gray-100 hover:text-gray-900 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-gray-950 disabled:pointer-events-none disabled:opacity-50 dark:border-gray-800 dark:bg-gray-950 dark:hover:bg-gray-800 dark:hover:text-gray-50 dark:focus-visible:ring-gray-300 roboto-flex azul-principal">
            Cancelar
        </a>
    </div>
    ...
    <section class="w-full py-12 flex justify-center">
        <div class="container grid items-center gap-6 px-4 md:px-6 lg:grid-cols-2 lg:gap-10">
            ...
            <img src="<?= url('storage/' . $alojamiento->imagen_alojamiento); ?>" width="550" height="400" alt="{{ $alojamiento->nombre_alojamiento}}" class="mx-auto aspect-[9/6] overflow-hidden rounded-xl object-cover object-center sm:w-full" />
            @else
