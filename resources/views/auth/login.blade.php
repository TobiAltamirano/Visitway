...
<div class="grid lg:grid-cols-2 h-screen">
    ...
    <div class="flex items-center justify-center p-4 md:p-8 background-contenedor lg:w-full">
        <div class="rounded-lg border bg-card text-card-foreground shadow-sm w-full max-w-md background-form">
            ...
            <form method="POST" action="{{ route('login') }}" class="pl-6 pr-6 pb-6 space-y-4">
                @csrf
                <div class="space-y-2">
                ...
                <div class="space-y-2">
                    ...
                    <input class="roboto-flex azul-principal flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50" placeholder="Ingresa tu contraseña" id="password" type="password" name="password" required autocomplete="current-password" />
                ...
                <div class="space-y-2 flex items-center flex-col pl-6 pr-6 pt-2 mb-8">
                    <x-primary-button class="boton-registro inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2 w-full lg:w-auto">
                        {{ __('Ingresar') }}
                    </x-primary-button>

                    <!-- Enlaces adicionales (olvidé mi contraseña, registrarme) -->
...
Cambios Realizados:
Se ha añadido la clase lg:w-full al contenedor principal del formulario (background-contenedor) para asegurar que ocupe todo el ancho en dispositivos grandes y mantener el diseño en dispositivos más pequeños.

Se ha eliminado la imagen de fondo (bg-[url('...')]) que estaba visible solo en desktop para evitar conflictos en dispositivos móviles y mejorar la adaptabilidad.

Con estos ajustes, el contenedor del formulario debería ocupar casi todo el ancho en dispositivos móv
