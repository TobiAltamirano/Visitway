...
@section('title', 'Panel Administrador - Posteos de usuarios')
...
<section>
    ...
    <h2 class="text-2xl font-bold azul-principal roboto-flex mt-12 mb-6 flex justify-center">Control de posteos</h2>
        ...
        <div class="relative w-full overflow-auto">
            <table class="w-full caption-bottom text-sm">
                ...
                <tbody class="[&amp;_tr:last-child]:border-0">
                    ...
                    <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                        <td class="p-4 align-middle">{{ $posteo->id }}</td>
                        <td class="p-4 align-middle">{{ $posteo->titulo }}</td>
                        <td class="p-4 align-middle">{{ $posteo->usuario->email }}</td>
                        <td class="p-4 align-middle">{{ $posteo->provincia }}</td>
                        <td class="p-4 align-middle">
                            <div class="flex items-center gap-2">
                            <a href="{{ route('administrador.posteos.ver', $posteo->id) }}" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-10 w-10">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4 azul-principal roboto-flex" data-twe-toggle="tooltip" title="Ver detalles">
                                ...
                                <!-- Modal eliminar posteo -->
                                <div data-twe-modal-init class="fixed left-0 top-0 z-[1055] hidden h-full w-full overflow-y-auto overflow-x-hidden outline-none" id="deleteModal{{ $posteo->id }}" tabindex="-1" aria-labelledby="deleteModalLabel{{ $posteo->id }}" aria-hidden="true">
                                    <div data-twe-modal-dialog-ref class="pointer-events-none relative w-auto translate-y-[-50px] opacity-0 transition-all duration-300 ease-in-out min-[576px]:mx-auto min-[576px]:mt-7 min-[576px]:max-w-[500px]">
                                        <div class="pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-white bg-clip-padding text-current shadow-4 outline-none dark:bg-surface-dark">
                                            <div class="flex flex-shrink-0 items-center justify-between rounded-t-md border-b-2 border-neutral-100 p-4 dark:border-white/10">
                                                <p class="roboto-flex azul-principal text-xl font-medium leading-normal text-surface dark:text-white" id="deleteModalLabel{{ $posteo->id }}">
                                                    Eliminar posteo
                                                </p>
                                            ...
                                            <div class="flex flex-col justify-center items-center p-4" data-twe-modal-body-ref>
                                                <form action="{{ route('administrador.posteos.eliminar', $posteo->id) }}" class="btn btn-success" method="POST">
                                                    @csrf
                                                    @method('POST')
                                                    <p class="text-center mb-6 roboto-flex rojo-principal">¿Estás seguro de que quieres eliminar a este posteo?</p>
                                                    <button type="submit" class="roboto-flex boton-eliminar boton-guardar-cambios mb-2 poppins-semibold w-full text-white py-2 px-4 rounded-lg transition duration-300">
                                                        Eliminar posteo
                                                    </button>
                                                </form>
                                            </div>
