...
<section>
    ...
    <div class="flex justify-center mt-8 gap-4">
        <div>
            <form action="{{ route('administrador.locales_gastronomicos.eliminar.procesar', ['id' => $localGastronomico->id_local_gastronomico]) }}" method="POST">
                @csrf
                @method('POST')
                <button class="inline-flex h-10 sm:w-auto items-center justify-center rounded-md bg-gray-900 px-8 text-sm font-medium text-gray-50 shadow transition-colors hover:bg-gray-900/90 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-gray-950 disabled:pointer-events-none disabled:opacity-50 dark:bg-gray-50 dark:text-gray-900 dark:hover:bg-gray-50/90 dark:focus-visible:ring-gray-300 boton-eliminar roboto-flex" type="submit" data-id="27">Eliminar local</button>
            </form>
        </div>
    ...
    <section class="w-full py-12 flex justify-center">
        <div class="container grid items-center gap-6 px-4 md:px-6 lg:grid-cols-2 lg:gap-10">
            ...
            <img src="<?= url('storage/imagenes/gastronomia/' . $localGastronomico->imagen_local_gastronomico); ?>" width="550" height="400" alt="{{ $gastronomia->nombre_local_gastronomico }}" class="mx-auto aspect-[9/6] overflow-hidden rounded-xl object-cover object-center sm:w-full" />
            @else
