...
<section>
    ...
    <h1 class="h1-provincias text-4xl text-2xl title-font mb-4 text-gray-900 rojo-secundario poppins-semibold">Editar alojamiento:<span class="span-h1-provincias"><br>{{ $alojamiento->nombre_alojamiento }}</span></h1>
            ...
            <div class="space-y-6">
                ...
                <form action="{{ route('administrador.alojamientos.editar.proceso', $alojamiento->id_alojamiento) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <p class="text-sm text-gray-500 azul-principal">Obligatorio</p>
                        <input id="contacto_alojamiento" name="contacto_alojamiento" class="roboto-flex azul-principal relative m-0 block w-full min-w-0 flex-auto rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50" value="{{ old('contacto_alojamiento') ?? $alojamiento->contacto_alojamiento }}" @error('contacto_alojamiento') aria-describedby="error-contacto_alojamiento" aria-invalid="true" @enderror></input>
